...
Edit [poems/poems.json](../poems/poems.json). Running `python3 scripts/build.py` regenerates:

- [README: seven featured grids](../README.md)
- [More poems: all 18 grids](../more_poems.md)
- [Selected rhyming poems](../poems/selected.md)
- [Complete ranked collection](../poems/collection.md)
- [Check results](../poems/checks.json)

Do not hand-edit those generated files. `python3 scripts/build.py --check` validates without writing and reports missing or stale outputs.

Python 3.9 or newer is required; there are no third-party dependencies. The poem data lives in `poems/poems.json`; presentation text and the seven featured IDs live in `PUBLIC_GROUPS` and the Markdown functions in [scripts/build.py](../scripts/build.py).
...
Ranks are the order of IDs in `sections`. Every current poem appears exactly once in the collection structure. Keep `status`, `curation.status`, the short assessment, and the fuller editorial notes consistent.

## Selection is a separate judgment

The README features three selected rhyming pairs, three selected symmetric squares, and one selected four-direction poem. Its seven-poem selection is explicit, rather than automatically changing with the rankings. `more_poems.md` contains every grid once, including those seven and the two unrhymed pairs, while marking reserve and workshop entries. Neither file replaces the detailed editorial collection.

The selected rhyming edition includes only entries in the rhyming collection marked `Selected`, with a passing rhyme check and different across/down readings. It excludes reserves, workshop drafts, and the explicitly conditional rhyme draft.
